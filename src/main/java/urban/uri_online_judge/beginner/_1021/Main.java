...
public class Main {
	...
	protected static String main(InputStream input) throws IOException {
		...
		sb = sb.append("MOEDAS:").append(LS);
		sb = sb.append((int) N).append(" moeda(s) de R$ 1.00").append(LS);
		N -= (int) N;
		...
		sb = sb.append(moedasDe50).append(" moeda(s) de R$ 0.50").append(LS);

		if (N / .25 >= 1.0) {
			moedasDe25 = (int) (N / 0.25);
			N -= moedasDe25 * 0.25;
		}
		sb = sb.append(moedasDe25).append(" moeda(s) de R$ 0.25").append(LS);

		if (N / .10 >= 1.0) {
			moedasDe10 = (int) (N / 0.10);
			N -= moedasDe10 * 0.10;
		}
		sb = sb.append(moedasDe10).append(" moeda(s) de R$ 0.10").append(LS);

		if (N / .05 >= 1.0) {
			moedasDe05 = (int) (N / 0.05);
			N -= moedasDe05 * 0.05;
		}
		sb = sb.append(moedasDe05).append(" moeda(s) de R$ 0.05").append(LS);

		moedasDe01 = (int) (Double.valueOf(formatter.format(N)) * 100);
		sb = sb.append(moedasDe01).append(" moeda(s) de R$ 0.01").append(LS);
